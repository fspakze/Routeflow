...
**ประวัติเส้นทาง (admin → แท็บ 🕘):** แก้ไข/ลบ trip ย้อนหลังได้ทุกสถานะ (ลบถาวร ลบลูก trip_stops/trip_crew/gps ให้ด้วย) · คุมด้วยสิทธิ์ `can_edit_trip`/`can_del_trip` (รัน **`trip_history_perms.sql`**)
